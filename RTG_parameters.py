...
def calc_block_length():  # Number of blocks that nearest to the apron side
    ...
    max_blocks_x = math.ceil((quay_length - rtg_traffic_lane) / (max_block_length + rtg_traffic_lane + 2 * rtg_margin_stack))

    'Determine block length"'
    block_length = math.floor((((quay_length - rtg_traffic_lane) / max_blocks_x)- rtg_traffic_lane - 2 * rtg_margin_stack)*100)/100
...
def calc_block_width (): # Number of blocks that nearest to the apron side
    ...
    block_width = 2 * rtg_track + rtg_vehicle_track + number_of_container_row * rtg_gross_tgs_y

    'Determine maximum number of blocks in row direction'
    max_blocks_y = math.floor((max(w) - apron_width + rtg_bypass_lane - rtg_traffic_lane) / (block_width + rtg_bypass_lane))
...
terminal_wotl_line = terminal_full_line.parallel_offset(rtg_traffic_lane, 'right', join_style=2, mitre_limit=rtg_traffic_lane)
...
ps_space = max(w) - apron_width + rtg_bypass_lane - rtg_traffic_lane - max_blocks_y * (block_width + rtg_bypass_lane)
...
while area_prim/area_terminal >= prim_total_ratio:
    ...
    line_y = math.ceil((line_y - block_width - rtg_bypass_lane)*100)/100
...
prim_yard_wotl_line = prim_yard_line.parallel_offset(rtg_traffic_lane, 'right', join_style=2, mitre_limit=rtg_traffic_lane)
...
'The starting origin for the container block is : x = rtg_traffic_lane + rtg_margin_stack ; y = apron_width'
starting_origin = [rtg_traffic_lane + rtg_margin_stack, apron_width]
