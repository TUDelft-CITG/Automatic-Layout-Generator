...
def generate_blocks(current_origin, block_length, block_width, block_location, container_location, i, m):
    ...
    if point_0_inside is True and point_1_inside is True and point_2_inside is True and point_3_inside is True:
        ...
        if block_location[0][0] == starting_origin[0] and block_location[0][1] == starting_origin[1] and m == 0:
            ...
            check_point = [[block_location[0][0] - block_length - 2 * rs_margin_stack - rs_traffic_lane, block_location[0][1]],
                           [block_location[0][0] - block_length - 2 * rs_margin_stack - rs_traffic_lane, block_location[0][1] + block_width]]
            ...
            if check_point_inside is True:
                ...
                check_point = [[block_location[0][0] - rs_traffic_lane - min_block_length, block_location[0][1]],
                               [block_location[0][0] - rs_traffic_lane - min_block_length, block_location[0][1] + block_width]]
                ...
                if check_point_inside is True:
                    ...
                    while point_0_inside is True and point_1_inside is True:
                        current_origin = [current_origin[0] - rs_gross_tgs_x, current_origin[1]]
                    ...
                    block_location[0] = [current_origin[0], current_origin[1]]
                    block_location[1] = [current_origin[0], current_origin[1] + block_width]
                    block_location[2] = [block_location[2][0] - block_length - 2 * rs_margin_stack - rs_traffic_lane, block_location[2][1]]
                    block_location[3] = [block_location[3][0] - block_length - 2 * rs_margin_stack - rs_traffic_lane, block_location[3][1]]
                    block_location[4] = [current_origin[0], current_origin[1]]
        ...
        if block_location[3][0] - block_location[0][0] > min_block_length:
            ...
            rectangle = plt.Rectangle(block_location[0], block_location[3][0] - block_location[0][0], block_width, fc='white', ec="black")
            plt.gca().add_patch(rectangle)

            'Calculates number of container in the container block on bay direction'
            number_of_container_bay = math.ceil((block_location[3][0] - block_location[0][0]) / rs_gross_tgs_x)
            ...
            container_clearance_y = rs_gross_tgs_y - container_y

            'Define new current origin for the algorithm in drawing the TGS'
            container_current_origin = block_location[0][0], block_location[0][1] + container_clearance_y / 2
        ...
        current_origin[0] = block_location[3][0] + rs_traffic_lane + 2 * rs_margin_stack
        ...
        if (point_1_inside is False and point_2_inside is False) or (point_1_inside is False and point_2_inside is False and point_3_inside is False) or (
                point_0_inside is False and point_2_inside is False and point_3_inside is False):
            'Define that it is possible to place on the next container block in bay direction'
            possible_placement = False

            'Define new current_origin point to the next container block in row direction'
            current_origin = [block_list[i][0][0], current_origin[1] + block_width + rs_operating_space]
            ...
            if point_0_inside is True and point_1_inside is True:
                ...
                'Define the points that needs to be checked'
                check_point = [[block_location[0][0] - rs_traffic_lane - 2 * rs_margin_stack - block_length, block_location[0][1]],
                               [block_location[0][0] - rs_traffic_lane - 2 * rs_margin_stack - block_length, block_location[0][1] + block_width]]
                ...
                if min(x) <= check_point[0][0]:
                    ...
                    'Check whether the right side already fulfill the requirments for rs_margin_stack'
                    check_point = [[block_location[0][0] - rs_margin_stack, block_location[0][1]],
                                   [block_location[0][0] - rs_margin_stack, block_location[0][1] + block_width]]
                    ...
                    while point_0_inside is True and point_1_inside is True and check_point_inside is True:
                        ...
                        'Check whether the right side already fulfill the requirments for rs_margin_stack'
                        check_point = [[block_location[0][0] - rs_margin_stack, block_location[0][0]],
                                       [block_location[0][0] - rs_margin_stack, block_location[0][0] + block_width]]
        ...
        if point_0_inside is True and point_1_inside is True and point_2_inside is True and point_3_inside is True:
            ...
            if block_location[3][0] - block_location[0][0] > min_block_length:
                ...
                'Check whether the right side already fulfill the requirments for rs_margin_stack'
                check_point = [[block_location[3][0] + rs_margin_stack, block_location[3][1]],
                               [block_location[3][0] + rs_margin_stack, block_location[3][1] + block_width]]
                ...
                while (point_2_inside is False or point_3_inside is False) and check_point_inside is False:
                    ...
                    block_location[0] = [current_origin[0], current_origin[1]]
                    block_location[1] = [current_origin[0], current_origin[1] + block_width]
                    block_location[2] = [block_location[2][0] - rs_gross_tgs_x, current_origin[1] + block_width]
                    block_location[3] = [block_location[3][0] - rs_gross_tgs_x, current_origin[1]]
                    block_location[4] = [current_origin[0], current_origin[1]]
