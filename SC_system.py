...
def generate_blocks(current_origin, block_length, block_width, block_location, container_location, i, m):
    ...
    if point_0_inside is True and point_1_inside is True and point_2_inside is True and point_3_inside is True:
        ...
        if block_location[0][0] == starting_origin[0] and block_location[0][1] == starting_origin[1] and m == 0:
            ...
            check_point = [[block_location[0][0] - block_width - sc_traffic_lane, block_location[0][1]],
                           [block_location[0][0] - block_width - sc_traffic_lane, block_location[0][1] + block_length]]
            ...
            if check_point_inside is True:
                ...
                check_point = [[block_location[0][0] - sc_traffic_lane - min_block_length, block_location[0][1]],
                               [block_location[0][0] - sc_traffic_lane - min_block_length, block_location[0][1] + block_length]]
                ...
                if check_point_inside is True:
                    ...
                    while point_0_inside is True and point_1_inside is True:
                        current_origin = [current_origin[0] - sc_gross_tgs_x, current_origin[1]]
                    ...
                    block_location[0] = [current_origin[0], current_origin[1]]
                    block_location[1] = [current_origin[0], current_origin[1] + block_length]
                    block_location[2] = [block_location[2][0] - block_width - sc_traffic_lane, block_location[2][1]]
                    block_location[3] = [block_location[3][0] - block_width - sc_traffic_lane, block_location[3][1]]
                    block_location[4] = [current_origin[0], current_origin[1]]
        ...
        if block_location[3][0] - block_location[0][0] > min_block_width and block_location[1][1] - block_location[0][1] > min_block_length:
            # Drawing rectangle for the representation of the blocks
            rectangle = plt.Rectangle(block_location[0], block_location[3][0] - block_location[0][0], block_location[1][1] - block_location[0][1], fc='white', ec="black")
            plt.gca().add_patch(rectangle)

            'Calculates number of container in a container block on bay direction'
            number_of_container_bay = math.floor((block_location[1][1] - block_location[0][1]) / sc_gross_tgs_y)

            'Calculates number of container in a contianer block on row direction'
            number_of_container_row = math.floor((block_location[3][0] - block_location[0][0]) / sc_gross_tgs_x)
            
            'Define clearance space for each container'
            container_clearance_x = ((block_location[3][0] - block_location[0][0]) - number_of_container_row * container_x) / (number_of_container_row - 1)
            container_clearance_y = ((block_location[1][1] - block_location[0][1]) - number_of_container_bay * container_y) / (number_of_container_bay - 1)
            ...
            for p in range (0,number_of_container_bay):
                for q in range (0, number_of_container_row):
        ...
        current_origin[0] = block_location[3][0] + sc_traffic_lane
        ...
        if (point_1_inside is False and point_2_inside is False) or (
                point_1_inside is False and point_2_inside is False and point_3_inside is False) or (point_0_inside is False and point_2_inside is False and point_3_inside is False):
            'Define that it is possible to place on the next container block in bay direction'
            possible_placement = False

            'Define new current_origin point to the next container block in row direction'
            current_origin = [block_list[i][0][0], current_origin[1] + block_length + sc_traffic_lane]

            'Determine the new block location on the new row direction'
            block_location[0] = [block_list[i][0][0], current_origin[1]]
            block_location[1] = [block_list[i][1][0], current_origin[1] + block_length]
            block_location[2] = [block_list[i][2][0], current_origin[1] + block_length]
            ...
            if point_1_inside is False and point_2_inside is False:
                if point_0_inside is True or point_3_inside is True:
                    while point_1_inside is False and point_2_inside is False:
            ...
            if point_0_inside is True and point_1_inside is True:
                ...
                'Define the points that needs to be checked'
                check_point = [[block_location[0][0] - sc_traffic_lane - block_width, block_location[0][1]],
                               [block_location[0][0] - sc_traffic_lane - block_width, block_location[0][1] + block_length]]
                ...
                if min(x) <= check_point[0][0]:
                    ...
                    'Check whether the right side already fulfill the requirments for sc_traffic_lane'
                    check_point = [[block_location[0][0] - sc_traffic_lane, block_location[0][1]],
                                   [block_location[0][0] - sc_traffic_lane, block_location[0][1] + block_length]]

                    check_point_inside = check_point_inside_com(check_point)

                    if point_0_inside is True and point_1_inside is True and check_point_inside is True:
                        while point_0_inside is True and point_1_inside is True and check_point_inside is True:
                            ...
                            'Check whether the right side already fulfill the requirments for sc_traffic_lane'
                            check_point = [[block_location[0][0] - sc_traffic_lane, block_location[0][0]],
                                           [block_location[0][0] - sc_traffic_lane, block_location[0][0] + block_width]]
            ...
            if point_0_inside is False or point_1_inside is False:
                ...
                'Define the points that needs to be checked'
                check_point = [[block_location[3][0] - min_block_width, block_location[3][1]],
                               [block_location[3][0] - min_block_width, block_location[3][1] + block_length]]

                'Check whether the check points is inside the terminal'
                check_point_inside = check_point_inside_com(check_point)

                if check_point_inside is False:

                    'Check whether it is fulfilling minimum block length'
                    'Define the points that needs to be checked'
                    check_point = [[block_location[0][0], block_location[0][1] + min_block_length],
                    ...
                    if check_point_inside is True:
                        while point_0_inside is False or point_1_inside is False:
                            ...
                            'Determine the new block location'
                            block_location[0] = [current_origin[0], current_origin[1]]
                            block_location[1] = [block_location[1][0], block_location[1][1] - sc_gross_tgs_y]
                            block_location[2] = [block_list[i][2][0], block_location[1][1] - sc_gross_tgs_y]
                            block_location[3] = [block_list[i][3][0], current_origin[1]]
                            block_location[4] = [current_origin[0], current_origin[1]]
        ...
        if block_location[3][0] < block_location[0][0]:
            ...
            if point_0_inside is False or point_1_inside is False:
                while point_0_inside is False or point_1_inside is False:
                    ...
                    block_location = [[current_origin[0], current_origin[1]],
                                      [current_origin[0], current_origin[1] + block_length],
                                      [block_list[i][2][0], current_origin[1] + block_length],
        ...
        if point_0_inside is True and point_1_inside is True and point_2_inside is True and point_3_inside is True:
            ...
            if block_location[3][0] - block_location[0][0] > min_block_length:
                ...
                check_point = [[block_location[3][0] + sc_traffic_lane, block_location[3][1]],
                               [block_location[3][0] + sc_traffic_lane, block_location[3][1] + block_length]]
                ...
                if check_point_inside is False:
                    ...
                    'Check whether it is fulfilling minimum block width'
                    'Define the points that needs to be checked'
                    check_point = [[block_location[0][0] + min_block_width, block_location[0][1]],
                                   [block_location[0][0] + min_block_width, block_location[0][1] + block_length]]
                    ...
                    if check_point_inside is True:
                        ...
                        if check_point_inside is True:
                            while point_1_inside is False or point_2_inside is False:
                                ...
                                block_location[0] = [current_origin[0], current_origin[1]]
                                block_location[1] = [block_location[1][0], block_location[1][1] - sc_gross_tgs_y]
                                block_location[2] = [block_location[2][0], block_location[2][1] - sc_gross_tgs_y]
                        ...
                        'Algorithm for : Decreasing the block length until it reaches the edge of the terminal in x- direction'
                        while (point_2_inside is False or point_3_inside is False):
                            'Determine the new block location'
                            block_location[0] = [current_origin[0], current_origin[1]]
                            block_location[1] = [current_origin[0], current_origin[1] + block_length]
                            block_location[2] = [block_location[2][0] - sc_gross_tgs_x, current_origin[1] + block_length]
                            block_location[3] = [block_location[3][0] - sc_gross_tgs_x, current_origin[1]]
                            block_location[4] = [current_origin[0], current_origin[1]]
